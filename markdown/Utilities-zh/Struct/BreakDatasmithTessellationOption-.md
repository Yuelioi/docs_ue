...
添加一个节点，将 "DatasmithTessellationOptions "分解为其成员字段。
...
Datasmith Tessellation Options。Datasmith Tessellation Options结构（通过引用）。 

## Outputs

Chord Tolerance。Float（单精度）。Chord Tolerance：。任何生成的三角形与原始表面之间的最大距离。较小的值会产生更多的三角形。
