...
添加一个节点，从其成员中创建一个 "SkeletalMeshBuildSettings"。
...
重新计算切线。布尔型。重新计算切线：。如果为真，原始网格中的切线会被忽略并重新计算。
...
Morph 阈值位置。Float（单精度）。Morph Threshold Position：.计算变形目标延迟时比较顶点位置是否相同的阈值。  
