# MakeModularSynthPreset

添加一个节点，从其成员中创建一个 "ModularSynthPreset"。

## 图示

![]($-20221218-14575019.png)
...
Osc 1Octave。Float（单精度）。Osc 1Octave:.振荡器1的倍频程。[-8.0, 8.0]。
...
增益Db。Float (single-precision).增益Db:。用于合成器的总体增益，单位是dB[-90.0, 20.0]。
...
立体声延迟时间。Float（单精度）。立体声延迟时间：。立体声延迟时间（ms）[0.0, 2000.0]。

立体声延时反馈。浮点（单精度）。立体声延时反馈：。立体声延时线的反馈量[0.0, 1.0]。

立体声延时湿电平。Float（单精度）。立体声延时湿电平：。用于立体声延迟时间的输出湿电平[0.0, 1.0]。

立体声延迟率。Float（单精度）。立体声延迟率：。左右立体声延迟线之间的比率（更宽的值是更多的分离）[0.0, 1.0]。

启用合唱。布尔型。Chorus Enabled:（启用合唱）。合唱效果是否被启用。
