# MakeSubmixEffectDynamicsProcesso-

添加一个节点，从其成员中创建一个 "SubmixEffectDynamicsProcessorSettings"。

## 图示

![]($-20221218-15024329.png)
...
关键的Lowshelf。Submix Effect动态处理器过滤器设置结构。按键低保：。对按键信号（如果提供了外部信号，或者没有提供输入信号）进行低档滤波器设置。  

## Outputs

Submix Effect Dynamics处理器设置。Submix Effect Dynamics Processor Settings结构。
