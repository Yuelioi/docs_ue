...
将序列评估器的时间设置为动画中距离曲线与输入的 DistanceToTarget 匹配的点。一个常见的用例是通过每一帧选择动画中与角色停止前剩余距离相匹配的点来实现无脚滑的停止。请注意，因为这个技术是通过剩余距离来设置动画时间的，所以它不尊重任何先前动画的相位（例如来自慢跑循环）。

目标是 Anim Distance Matching Library

## 图示

![](/uploads/projects/ue-bluprint/20221218-18431501.png)
