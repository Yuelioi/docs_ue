# SetCubemap

当 SourceType 设置为 SpecifiedCubemap 时，设置所使用的 cubemap，并在下次勾选时引起 skylight 更新。

目标是天灯组件

## 图示

![](/uploads/projects/ue-bluprint/20221218-20583742.png)

## Inputs

在。执行。

目标。天空之光组件对象参考。

新的立方体地图。纹理立方体对象参考。

## Outputs

出：执行。

<hr>

Sets the cubemap used when SourceType is set to SpecifiedCubemap, and causes a skylight update on the next tick.

Target is Sky Light Component

## 图示

![](/uploads/projects/ue-bluprint/20221218-20583742.png)

## Inputs

In: Exec.

Target: Sky Light Component Object Reference.

New Cubemap: Texture Cube Object Reference.

## Outputs

Out: Exec.
