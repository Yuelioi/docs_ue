...
包裹 AInstancedFoliageActor::GetInstancedFoliageActorForLevel，并允许检索当前 AInstancedFoliageActor。如果 Level 是空的，将默认为当前的编辑器级别。这个函数很有用，因为否则很难检索到这个角色，因为它将从 EditorLevelLibrary.get_all_level_actors()等函数的结果中被过滤掉。

目标是 Usd Exporter 蓝图库

## 图示

![](/uploads/projects/ue-bluprint/20221218-21211802.png)
...
返回值。实例叶子演员对象参考。包裹 AInstancedFoliageActor::GetInstancedFoliageActorForLevel，并允许检索当前 AInstancedFoliageActor。如果 Level 是空的，将默认为当前的编辑器级别。这个函数很有用，因为否则很难检索到这个角色，因为它将从 EditorLevelLibrary.get_all_level_actors()等函数的结果中被过滤掉。
