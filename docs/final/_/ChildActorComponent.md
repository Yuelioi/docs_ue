...
设置子演员类别。设置用于子演员的类。如果在一个模板组件上调用（由 CDO 拥有），任何现有的子演员模板的属性将被尽可能地复制到模板上。如果在一个世界中的组件实例上调用（并且该类正在改变），创建的儿童演员将使用模板的默认类。目标是子角色组件。
