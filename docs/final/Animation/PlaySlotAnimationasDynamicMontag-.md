# PlaySlotAnimationasDynamicMontag-

通过创建一个动态的 UAnimMontage，在槽节点上播放正常的动画资产。每个 SlotGroup 一次只能播放一个资产（无论是蒙太奇还是动画序列）。

目标是 Anim Instance

## 图示

![](/uploads/projects/ue-bluprint/20221218-17502422.png)
...
混合输出触发时间。Float (single-precision).
...
返回值。动画蒙太奇对象参考。通过创建一个动态的 UAnimMontage 在槽节点上播放普通的动画资产。每个槽组一次只能播放一个资产（无论是蒙太奇还是动画序列）。
