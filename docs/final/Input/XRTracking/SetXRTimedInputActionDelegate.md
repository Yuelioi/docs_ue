...
钩住一个委托来获取 OpenXR 动作事件的动作时间。对于布尔输入，委托的 "值 "参数在按下时是 1.0，释放时是 0.0。对于一个模拟输入，其值的范围是动作和平台特定的。与 GetControllerTransformForTime 结合使用，可以提高时间变换的精度和速度数据。"Left Grip "是一个有效的动作名称的例子。注意：这可能会在未来的某个时候被核心输入系统中对事件时间的本地支持所取代。

目标是头戴式显示器功能库

## 图示

![](/uploads/projects/ue-bluprint/20221218-19302447.png)
