...
制作一个横跨给定的 EAudioSpectrumBandPresetType 的频率范围的频段阵列。

目标是音频混合器蓝图库

## 图示

![](/uploads/projects/ue-bluprint/20221218-18022587.png)

## Inputs

在波段预设类型中。EAudioSpectrumBandPresetType 枚举。带子所包含的音频内容类型。

In Num Bands: 整数。用来表示频谱的频带数量。

在攻击时间 Msec。整数。应用于每个波段的包络跟踪器的攻击时间（以毫秒为单位）。
