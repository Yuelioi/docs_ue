...
检索当前播放时间的熟化振幅包络数据。如果有多个。声波在播放，数据会在所有播放的声波中进行内插并取平均值。如果没有找到数据，返回 FALSE。

目标是音频组件

## 图示

![](/uploads/projects/ue-bluprint/20221218-18025520.png)
...
返回值。布尔值。检索当前播放时间的熟化振幅包络数据。如果有多个。播放的声波，数据将在所有播放的声波中进行内插和平均。如果没有找到数据，则返回 FALSE。
