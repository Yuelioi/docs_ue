# GetChannelBufferOverflowStat

返回自上次重置以来该输入检测到的缓冲区溢出的数量。

目标是定时数据监控子系统

## 图示

![](/uploads/projects/ue-bluprint/20221218-21121958.png)

## Inputs

在。执行。

目标。定时数据监控子系统对象参考。

Identifier:定时数据监控通道标识符结构（通过参考）。

## Outputs

出：执行。

返回值。整数。返回自上次重置以来该输入检测到的缓冲区溢出的数量。

<hr>

Returns the number of buffer overflows detected by that input since the last reset.

Target is Timed Data Monitor Subsystem

## 图示

![](/uploads/projects/ue-bluprint/20221218-21121958.png)

## Inputs

In: Exec.

Target: Timed Data Monitor Subsystem Object Reference.

Identifier: Timed Data Monitor Channel Identifier Structure (by ref).

## Outputs

Out: Exec.

Return Value: Integer. Returns the number of buffer overflows detected by that input since the last reset..
