...
在一个后台任务中，将图像转换为 JPEG 数据阵列

目标是 Apple Image Utils Base Async Task Blueprint Proxy

## 图示

![](/uploads/projects/ue-bluprint/20221218-19164045.png)
...
想要颜色：布尔值。JPEG 是彩色（真）还是单色（假）。

Use Gpu: 布尔值。是否使用 GPU（真）或 CPU（假）来压缩。

缩放。Float（单精度）。是否在转换前缩放图像，默认为不缩放。

Rotate（旋转）。ETextureRotationDirection 枚举。在转换过程中对图像进行旋转的方向，默认为无。
...
转换结果。Apple Image Utils 图像转换结果结构（按参考）。
...
Want Color: Boolean. whether the JPEG is color (true) or monochrome (false).
