...
要求电影管道在下一个可用的时间内关闭。该流水线将尝试放弃当前帧（例如，如果有更多的时间样本等待处理），但如果有空间样本已经提交给 GPU，则可能被迫结束。关闭流程将被运行以确保已经完成的工作被写入磁盘。这是一个非阻塞操作，如果你需要的话，请使用 Shutdown()来代替。

目标是电影管道

## 图示

![](/uploads/projects/ue-bluprint/20221218-20095070.png)
