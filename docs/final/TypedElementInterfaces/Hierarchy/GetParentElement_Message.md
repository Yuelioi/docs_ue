# GetParentElement_Message

获取这个元素的逻辑父级，如果有的话。 eg）一个组件可能会返回它的角色，或者一个静态网格实例可能会返回它的 ISM 组件。

目标是类型化的元素层次结构界面

## 图示

![](/uploads/projects/ue-bluprint/20221218-21173612.png)
...
返回值。脚本类型的元素处理结构。获取该元素的逻辑父级，如果有的话。例如）一个组件可能会返回它的角色，或者一个静态网格实例可能会返回它的 ISM 组件。
