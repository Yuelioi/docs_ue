...
![](/uploads/projects/ue-bluprint/20221218-19085782.png)
...
返回值。Float（单精度）。原始访问器用于询问游戏效果的大小，不一定总是正确的。外部代码（UI 等）应该如何询问 "这个游戏效果对我的伤害有多大 "这样的问题。 最有可能的是，我们想在后台捕捉到这个问题--当伤害被应用时，我们可以得到一个完整的转储/历史，了解这个数字是如何达到这个程度的。但我们仍然可能需要像下面这样的投票方法（我的伤害会是多少）。
