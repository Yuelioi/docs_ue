...
将时间码转换为字符串（hh:mm:ss:ff）。如果 bForceSignDisplay，那么数字符号将总是被预置，而不仅仅是在表达负数时间时。

目标是时间管理蓝图库

## 图示

![](/uploads/projects/ue-bluprint/20221218-15251249.png)
...
返回值。字符串。将时间码转换为字符串（hh:mm:ss:ff）。如果 bForceSignDisplay，那么数字符号将始终被添加，而不仅仅是在表达负的时间时。
