# ToText_float

根据格式化选项将传入的浮动数据转换为文本

目标是 Kismet 文本库

## 图示

![](/uploads/projects/ue-bluprint/20221218-15242576.png)

## Inputs

值。Float（单精度）。

四舍五入模式。ERoundingMode Enum.

总是签名：布尔值。

使用分组。布尔型。

最小整数位。整数。

最大整数位。整数。

最小的小数位。整数。

最大的小数位。整数。

## Outputs

返回值。文本。根据格式化选项，将传入的浮点数转换为文本。

<hr>

Converts a passed in float to text based on formatting options

Target is Kismet Text Library

## 图示

![](/uploads/projects/ue-bluprint/20221218-15242576.png)

## Inputs

Value: Float (single-precision).

Rounding Mode: ERoundingMode Enum.

Always Sign: Boolean.

Use Grouping: Boolean.

Minimum Integral Digits: Integer.

Maximum Integral Digits: Integer.

Minimum Fractional Digits: Integer.

Maximum Fractional Digits: Integer.

## Outputs

Return Value: Text. Converts a passed in float to text based on formatting options.
