# SetCollisionProfileName

设置碰撞配置文件名称。当构造函数设置 ProfileName 时，会调用该函数。这将把当前的 CollisionProfileName 改成这个，并覆盖 Collision Setting。

目标是原始组件

## 图示

![](/uploads/projects/ue-bluprint/20221218-18194677.png)

## Inputs

在。执行。

目标。原始组件对象参考。

在碰撞配置文件名称。名称： 新配置文件名称。

更新重叠。Boolean（布尔）。

## Outputs

输出。执行：执行。

<hr>

Set Collision Profile Name. This function is called by constructors when they set ProfileName. This will change current CollisionProfileName to be this, and overwrite Collision Setting

Target is Primitive Component

## 图示

![](/uploads/projects/ue-bluprint/20221218-18194677.png)

## Inputs

In: Exec.

Target: Primitive Component Object Reference.

In Collision Profile Name: Name. : New Profile Name.

Update Overlaps: Boolean.

## Outputs

Out: Exec.
