...
清除内部网格为一个 100x100x100 的立方体，基点在原点。如果不希望有一个初始空的网格，可以用这个方法代替 Reset()（例如对于一个组件）。

目标是动态网格

## 图示

![](/uploads/projects/ue-bluprint/20221218-18454100.png)
...
目标。动态网格对象参考。清除内部网格为一个 100x100x100 的立方体，基点在原点。如果不希望有一个最初为空的网格，可以用这个方法代替 Reset()（例如对于一个组件）。
