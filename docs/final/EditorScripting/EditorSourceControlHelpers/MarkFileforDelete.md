...
使用当前设置的源码控制提供者将文件从源码控制中删除，并删除该文件。注意：阻断直到行动完成。

目标是源代码控制帮助器

## 图示

![](/uploads/projects/ue-bluprint/20221218-18501284.png)

## Inputs

在。执行。

在文件中：字符串。要删除的文件 - 可以是完全合格的路径、相对路径、长包名称、资产路径或导出文本路径（通常存储在剪贴板上）。
...
In File: String. The file to delete - can be either fully qualified path, relative path, long package name, asset path or export text path (often stored on clipboard).
