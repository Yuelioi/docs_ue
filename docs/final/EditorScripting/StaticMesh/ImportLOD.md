...
导入或重新导入一个 LOD 到指定的基本网格。如果 LOD 不存在，它将导入它并将其添加到基础静态网格。如果 LOD 已经存在，它将重新导入指定的 LOD。

目标是静态网格编辑器子系统

## 图示

![](/uploads/projects/ue-bluprint/20221218-18540939.png)
...
返回值。整数。被导入或重新导入的 LOD 的索引。如果出现问题，将返回 INDEX_NONE。
