...
![](/uploads/projects/ue-bluprint/20221218-18542428.png)

## Inputs

在。执行。

目标数组。Actor 对象的数组引用。要过滤的 Actor 数组。该数组不会改变。

等级名称。名称。行为者所属等级的名称（与 ContentBrowser 中的名称相同）。

过滤器类型。EEditorScriptingFilterType 枚举。如果它符合条件，应该包括或不包括数组的项目。
