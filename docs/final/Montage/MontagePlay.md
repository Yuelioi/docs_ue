...
播放一个蒙太奇动画。返回动画蒙太奇的长度（秒）。如果播放失败，返回 0.f。

目标是 Anim Instance

## 图示

![](/uploads/projects/ue-bluprint/20221218-20071168.png)
...
返回值。浮点数（单精度）。播放一个动画蒙太奇。返回动画蒙太奇的长度（秒）。如果播放失败，返回 0.f。
...
Return Value: Float (single-precision). Plays an animation montage. Returns the length of the animation montage in seconds. Returns 0.f if failed to play..
