...
On Media Opened:.当一个媒体源被打开时被调用的一个委托。

根据底层播放器实现是同步还是异步打开媒体，这个事件可能在调用 OpenSource/OpenUrl 返回之前或之后执行。

@见 OnMediaOpenFailed, OnTracksChanged

<hr>

On Media Opened:. A delegate that is invoked when a media source has been opened.
