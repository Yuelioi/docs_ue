# GetDirectionatTime

给定一个从 0 到花键持续时间的时间，返回花键在那里的切线的单位方向向量。

目标是花键组件

## 图示

![](/uploads/projects/ue-bluprint/20221218-21002726.png)

## Inputs

目标。花键组件对象参考。

时间。浮点数（单精度）。

坐标空间。ESplineCoordinateSpace Enum.

使用恒定速度。布尔型。

## Outputs

返回值。向量。给定一个从 0 到花键持续时间的时间，返回花键切线的单位方向向量。

<hr>

Given a time from 0 to the spline duration, return a unit direction vector of the spline tangent there.

Target is Spline Component

## 图示

![](/uploads/projects/ue-bluprint/20221218-21002726.png)

## Inputs

Target: Spline Component Object Reference.

Time: Float (single-precision).

Coordinate Space: ESplineCoordinateSpace Enum.

Use Constant Velocity: Boolean.

## Outputs

Return Value: Vector. Given a time from 0 to the spline duration, return a unit direction vector of the spline tangent there..
