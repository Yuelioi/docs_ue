...
给出沿该花键长度的一个距离，返回该花键在那里的滚动，单位是度。

目标是花键组件

## 图示

![](/uploads/projects/ue-bluprint/20221218-21004795.png)
...
返回值。浮点数（单精度）。给出沿花键长度的一个距离，返回花键在那里的滚动，单位是度。
...
Return Value: Float (single-precision). Given a distance along the length of this spline, return the spline's roll there, in degrees..
