...
给出一个从 0 到花键持续时间的时间，返回花键在那里的滚动，单位是度。

目标是花键组件

## 图示

![](/uploads/projects/ue-bluprint/20221218-21005136.png)
...
返回值。Float（单精度）。给定一个从 0 到花键持续时间的时间，返回花键在那里的滚动，单位是度。
...
Return Value: Float (single-precision). Given a time from 0 to the spline duration, return the spline's roll there, in degrees..
