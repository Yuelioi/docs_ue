...
这是用于设置物理学混合重量的全局设置。这将自动做很多事情。如果PhysicsBlendWeight == 1.f，它将启用模拟，如果PhysicsBlendWeight == 0.f，它将禁用模拟。同时，它将尊重每个体的设置，所以如果体是固定的，它将不会模拟。反之亦然。所以如果你想让所有的体都手动改变，不要使用这个函数，而是使用SetAllBodiesPhysicsBlendWeight。
