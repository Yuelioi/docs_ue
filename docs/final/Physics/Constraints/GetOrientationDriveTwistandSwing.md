...
获取是否启用了角向驱动。只有在AngularDriveMode被设置为Twist和Swing时才相关。

目标是约束实例蓝图库

## 图示

![]($-20221218-20270316.png)
...
输出 启用扭转驱动。布尔值。表示是否启用了扭曲轴的驱动。只有在AngularDriveMode设置为Twist和Swing时才相关。

Out 启用摇摆驱动。布尔值。表示是否启用了摆动轴的驱动。只有在AngularDriveMode被设置为Twist和Swing时才相关。
