# SetAllMotorsAngularPositionDrive

启用或禁用 AngularPositionDrive。如果电机处于 SLERP 模式，如果 EnableSwingDrive 或 EnableTwistDrive 被启用，它将被打开。在扭转和摆动模式下，扭转和摆动可以单独控制。

目标是骨架网组件

## 图示

![](/uploads/projects/ue-bluprint/20221218-20282765.png)

## Inputs

在。执行。

目标。骨骼网组件对象参考。

启用摇摆驱动。布尔值。

启用扭转驱动。布尔型。

跳过自定义物理类型。布尔值。

## Outputs

出：执行。

<hr>

Enable or Disable AngularPositionDrive. If motor is in SLERP mode it will be turned on if either EnableSwingDrive OR EnableTwistDrive are enabled. In Twist and Swing mode the twist and the swing can be controlled individually.

Target is Skeletal Mesh Component

## 图示

![](/uploads/projects/ue-bluprint/20221218-20282765.png)

## Inputs

In: Exec.

Target: Skeletal Mesh Component Object Reference.

Enable Swing Drive: Boolean.

Enable Twist Drive: Boolean.

Skip Custom Physics Type: Boolean.

## Outputs

Out: Exec.
