...
获取系统实例的固定边界。这将返回用户设置的固定边界，如果设置了，则返回系统的固定边界。注意：如果不存在固定界限，返回的框可能是无效的。

目标是 Niagara 粒子系统组件

## 图示

![](/uploads/projects/ue-bluprint/20221218-20121718.png)
...
返回值。箱体结构。获取系统实例的固定边界。如果设置了固定界限，这将返回用户设置的固定界限；如果设置了固定界限，则返回系统的固定界限。注意：如果没有固定边界存在，返回的盒子可能是无效的。
