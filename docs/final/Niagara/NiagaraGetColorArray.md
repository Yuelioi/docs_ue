# NiagaraGetColorArray

获取Niagara FLinearColor Data的副本。

目标是Niagara数据接口阵列函数库

## 图示

![]($-20221218-20122998.png)

## Inputs

在。执行。

尼亚加拉系统。Niagara Particle System组件对象参考。

覆盖名称：名称。  

## Outputs

出：执行。

返回值。线性颜色结构的数组。获取Niagara FLinearColor数据的副本。

Gets a copy of Niagara FLinearColor Data.

Target is Niagara Data Interface Array Function Library

## 图示

![]($-20221218-20122998.png)

## Inputs

In: Exec.

Niagara System: Niagara Particle System Component Object Reference.

Override Name: Name.  

## Outputs

Out: Exec.

Return Value: Array of Linear Color Structures. Gets a copy of Niagara FLinearColor Data..

