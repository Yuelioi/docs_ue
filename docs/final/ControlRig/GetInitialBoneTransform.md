# GetInitialBoneTransform

返回骨骼在请求空间中的初始变换。

目标是控制钻机组件

## 图示

![](/uploads/projects/ue-bluprint/20221218-18313811.png)

## Inputs

目标。控制钻机组件对象参考。

骨骼名称。名称。要检索转换的骨骼名称。

空间。EControlRigComponentSpace 枚举。检索变换的空间。

## Outputs

返回值。Transform：骨骼在请求空间中的初始变换。

<hr>

Returns the initial transform of the bone in the requested space

Target is Control Rig Component

## 图示

![](/uploads/projects/ue-bluprint/20221218-18313811.png)

## Inputs

Target: Control Rig Component Object Reference.

Bone Name: Name. The name of the bone to retrieve the transform for.

Space: EControlRigComponentSpace Enum. The space to retrieve the transform in.

## Outputs

Return Value: Transform. the initial transform of the bone in the requested space.
