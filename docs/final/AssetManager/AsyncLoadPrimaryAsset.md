...
将一个主要的资产对象加载到内存中，这将导致它一直被加载，直到它被明确卸载。当加载成功或失败时，完成事件就会发生，你应该对加载的对象进行铸造，以验证它是正确的类型。如果指定了 LoadBundles，这些包会和资产一起被加载。

目标是 Async Action Load Primary Asset

## 图示

![](/uploads/projects/ue-bluprint/20221218-17595767.png)

## Inputs

在。执行。

Primary Asset: 主要资产 ID 结构。
