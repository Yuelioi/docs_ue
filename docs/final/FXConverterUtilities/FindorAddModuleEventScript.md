...
找到一个模块脚本转换上下文，或为事件类别添加一个模块脚本转换上下文到这个发射器转换上下文。如果通过名称字符串没有找到一个脚本转换上下文，那么将从 NiagaraScriptAssetData 中创建并初始化一个新的。

目标是尼亚加拉发射器转换背景

## 图示

![](/uploads/projects/ue-bluprint/20221218-19014141.png)
...
返回值。尼亚加拉脚本转换上下文对象参考。找到一个模块脚本转换上下文，或为事件类别的这个发射器转换上下文添加一个模块脚本转换上下文。如果通过名称字符串没有找到一个脚本转换上下文，那么将从 NiagaraScriptAssetData.中创建并初始化一个新的脚本转换上下文。
