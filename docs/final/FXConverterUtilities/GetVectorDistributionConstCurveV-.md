# GetVectorDistributionConstCurveV-

获取矢量分布常态曲线值

目标是 FXConverter Utilities Library

## 图示

![](/uploads/projects/ue-bluprint/20221218-19031163.png)

## Inputs

在。执行。

分布。Distribution Vector Constant Curve 对象参考。

## Outputs

出：执行。

Out Interp Curve Vector:Interp Curve Vector 结构。

<hr>

Get Vector Distribution Const Curve Values

Target is FXConverter Utilities Library

## 图示

![](/uploads/projects/ue-bluprint/20221218-19031163.png)

## Inputs

In: Exec.

Distribution: Distribution Vector Constant Curve Object Reference.

## Outputs

Out: Exec.

Out Interp Curve Vector: Interp Curve Vector Structure.
