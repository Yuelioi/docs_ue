...
返回指定时间的精灵（以秒为单位），如果不存在，则返回 nullptr。当 bClampToEnds 为真时，如果时间超出范围，它将选择第一个或最后一个精灵。

目标是纸质翻页书

## 图示

![](/uploads/projects/ue-bluprint/20221218-21022566.png)
...
返回值。纸质精灵对象参考。返回指定时间的精灵（单位：秒），如果不存在，则返回 nullptr。当 bClampToEnds 为真时，如果时间超出范围，它将选择第一个或最后一个精灵。
