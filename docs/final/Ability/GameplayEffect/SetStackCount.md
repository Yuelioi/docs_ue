# SetStackCount

将 GameplayEffectSpec 的 StackCount 设置为指定数量（在应用之前）。

目标是能力系统蓝图库

## 图示

![](/uploads/projects/ue-bluprint/20221218-17315747.png)

## Inputs

在。执行。

Spec Handle。Gameplay Effect Spec Handle 结构。

堆栈计数。整数。

## Outputs

输出。执行：执行。

返回值。Gameplay Effect Spec Handle 结构。将 GameplayEffectSpec 的 StackCount 设置为指定数量（应用前）。

<hr>

Sets the GameplayEffectSpec's StackCount to the specified amount (prior to applying)

Target is Ability System Blueprint Library

## 图示

![](/uploads/projects/ue-bluprint/20221218-17315747.png)

## Inputs

In: Exec.

Spec Handle: Gameplay Effect Spec Handle Structure.

Stack Count: Integer.

## Outputs

Out: Exec.

Return Value: Gameplay Effect Spec Handle Structure. Sets the GameplayEffectSpec's StackCount to the specified amount (prior to applying).
