...
等待，直到一个游戏效果被应用到目标角色身上。如果 TriggerOnce 为真，这个动作将只激活一次。否则，在能力的生命周期内，每次应用 GE 时，它都会返回，并满足要求

目标是能力 异步等待应用游戏效果

## 图示

![](/uploads/projects/ue-bluprint/20221218-17300744.png)
...
源过滤器。Gameplay Target Data Filter Handle 结构。

Source Tag Requirements（源标签要求）。Gameplay Tag Requirements 结构。
