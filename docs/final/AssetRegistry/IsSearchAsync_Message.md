# IsSearchAsync_Message

搜索是以异步方式进行（并且在启动时被启动），还是同步和按需进行，需要ScanPathsSynchronous或SearchAllAssets。
...
返回值。布尔值。搜索是以异步方式进行（在启动时启动），还是以同步方式按需进行，需要ScanPathsSynchronous或SearchAllAssets。
