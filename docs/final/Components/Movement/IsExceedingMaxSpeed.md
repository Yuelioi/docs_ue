...
如果当前速度超过了给定的最大速度（通常是 GetMaxSpeed()的结果），在一个很小的误差范围内，返回真。请注意，在正常情况下，由加速度引起的更新不会导致这个结果为真，但是外力或最大速度限制的变化会导致最大速度被违反。

目标是运动组件

## 图示

![](/uploads/projects/ue-bluprint/20221218-18245952.png)
...
返回值。布尔值。如果当前速度超过了给定的最大速度（通常是 GetMaxSpeed()的结果），在一个很小的误差范围内，返回真。请注意，在正常情况下，由加速度引起的更新不会导致该值为真，但是外力或最大速度限制的变化会导致最大速度被违反。
