# UnHideBonebyName

解除对指定骨骼的名称隐藏。目前，这只是对隐藏的骨骼强制执行一个 0 的比例。与 HideBone By Index 相比--它可以跟踪骨骼列表并在 LOD 变化时进行更新。

目标是带皮的网格组件

## 图示

![](/uploads/projects/ue-bluprint/20221218-18272288.png)

## Inputs

在。执行。

目标。剥皮网状构件对象参考。

骨骼名称。名称。要取消隐藏的骨骼名称。

## Outputs

输出。执行：执行。

<hr>

UnHide the specified bone with name. Currently this just enforces a scale of 0 for the hidden bones.. Compared to HideBone By Index - This keeps track of list of bones and update when LOD changes

Target is Skinned Mesh Component

## 图示

![](/uploads/projects/ue-bluprint/20221218-18272288.png)

## Inputs

In: Exec.

Target: Skinned Mesh Component Object Reference.

Bone Name: Name. Name of bone to unhide.

## Outputs

Out: Exec.
