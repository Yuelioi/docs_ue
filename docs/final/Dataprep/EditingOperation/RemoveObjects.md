...
从 Dataprep 的和/或动作的工作集中删除一个对象阵列

目标是 Dataprep 编辑操作

## 图示

![](/uploads/projects/ue-bluprint/20221218-18354018.png)
...
对象。对象引用的数组。要从工作集移除的对象数组。

本地环境。布尔值。如果设置为 "true"，该对象将从当前工作集中移除。该对象将不会被任何使用当前上下文的后续操作所访问。如果设置为 false，该对象将从 Dataprep 的工作集中移除。该对象将不会被 Dataprep 的管道中的任何后续操作所访问。
