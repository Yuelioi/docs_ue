...
注意：为了使物理模拟过程中的碰撞产生撞击事件，必须启用 "模拟产生撞击事件"。注意：当从另一个物体的运动中收到撞击时（bSelfMoved 是假的），'Hit.Normal'和'Hit.ImpactNormal'的方向将被调整以表示来自另一个物体对这个物体的力。注意：对于物理模拟体来说，NormalImpulse 将被填入，但是对于扫荡组件的阻挡碰撞来说，NormalImpulse 将为零。

目标是演员

## 图示

![](/uploads/projects/ue-bluprint/20221218-17450401.png)
