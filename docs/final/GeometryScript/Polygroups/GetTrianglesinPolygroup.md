...
在给定的 GroupLayer 中创建具有给定的 Polygroup ID 的所有三角形的列表（不一定是单一的连接构件）。

目标是几何脚本库网格多组函数

## 图示

![](/uploads/projects/ue-bluprint/20221218-19124412.png)
...
三角形 ID 出来：索引列表结构（按参考）。

## Outputs

出：执行。

目标网格。动态网格对象参考。创建给定 GroupLayer 中具有给定 Polygroup ID 的所有三角形的列表（不一定是单一的连接构件）。
