...
![](/uploads/projects/ue-bluprint/20221218-19074883.png)
...
碰撞处理重写。ESpawnActorCollisionHandlingMethod 枚举。指定如何处理产卵点的碰撞。如果未定义，则使用角色类设置。

所有者：行为者对象参考。可以留空；主要用于复制（bNetUseOwnerRelevancy 和 bOnlyRelevantToOwner），或可见性（PrimitiveComponent 的 bOwnerNoSee/bOnlyOwnerSee）。

## Outputs

出：执行。

返回值。Actor 对象参考。Actor Object Reference 返回值。构建的对象。
