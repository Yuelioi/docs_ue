...
根据另外两个材质实例和一个 alpha 混合因子，插值这个材质实例的标量和矢量参数。输出是对象本身（this）。支持 SourceA==this || SourceB==this 的情况。这两种材料必须来自同一个基础材料。

目标是材料实例动态

## 图示

![](/uploads/projects/ue-bluprint/20221218-20380315.png)
...
来源 A：材料实例对象参考。Alpha=0 时使用的值，如果是 0，就会默默地忽略这种情况。

来源 B。材料实例对象参考。Alpha=1 时使用的值，如果是 0，就会默默地忽略这种情况。

阿尔法。浮点数（单精度）。通常在 0...1 的范围内，超出该范围的数值会被外推。
