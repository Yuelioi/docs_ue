...
从云端加载一个引脚... 这将启动一个 Latent Action 来加载一个云锚，并为它创建一个针。

目标是 Azure 空间锚点库

## 图示

![](/uploads/projects/ue-bluprint/20221218-17555325.png)

## Inputs

在。执行。

云标识符。字符串。我们将尝试加载的云锚的 Azure 云空间锚标识符。

引脚标识。字符串。指定要加载的引脚的名称。如果该名称的引脚已经存在，则会失败。如果为空，将使用自动生成的 ID。

## Outputs

已完成。Exec.

Out ARPin: ARPin 对象参考。如果成功的话，用创建的引脚来填充。

Out Azure Cloud Spatial Anchor: Azure Cloud Spatial Anchor Object Reference。填写创建的云空间锚的 UE 表示，如果成功。
