...
返回文本转语音对屏幕阅读器用户说话的音量。值在 0.0f 和 1.0f 之间。如果提供的用户 ID 不存在，将返回 0.0f。

目标是 Slate 屏幕阅读器引擎子系统

## 图示

![](/uploads/projects/ue-bluprint/20221218-20584551.png)

## Inputs

目标。石板屏幕阅读器引擎子系统对象参考。

读屏器用户标识。整数。检索文字转语音音量的屏幕阅读器用户的 ID。
...
Screen Reader User Id: Integer. The user Id of the screen reader user to retrieve the text to speech volume for.
