# SetMaterialParameter

在这个 ParticleSystemComponent 上设置一个命名的材质实例参数。如果该参数已经存在，则更新它，如果不存在，则创建一个新条目。

目标是粒子系统组件

## 图示

![](/uploads/projects/ue-bluprint/20221218-18550141.png)

## Inputs

在。执行。

目标。粒子系统组件对象参考。

参数名称： 名称。

参数：材料界面对象参考。

## Outputs

出：执行。

<hr>

Set a named material instance parameter on this ParticleSystemComponent.. Updates the parameter if it already exists, or creates a new entry if not.

Target is Particle System Component

## 图示

![](/uploads/projects/ue-bluprint/20221218-18550141.png)

## Inputs

In: Exec.

Target: Particle System Component Object Reference.

Parameter Name: Name.

Param: Material Interface Object Reference.

## Outputs

Out: Exec.
