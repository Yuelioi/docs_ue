...
从原始像素数据创建一个 ARCandidateImage 对象，并将其添加到给定的\c UARSessionConfig 对象的 ARCandidateImageList 中。

注意，你需要用你所添加的 UARSessionConfig 重新启动 AR 会话，以使变化生效。

在 ARCore 平台上，如果你不知道图像的物理尺寸，或者物理尺寸是动态的，你可以把 PhysicalWidth 和 PhysicalHeight 设为 0。而且这个函数需要时间来执行非琐碎的图像处理（20ms - 30ms），.应该在后台线程上运行。

目标是谷歌 ARCore 会话函数库

## 图示

![](/uploads/projects/ue-bluprint/20221218-17573478.png)
...
返回值。ARCandidate Image Object Reference。A /c UARCandidateImage 对象指针，如果底层的 ARPlatform 在运行时成功添加了候选图像。否则返回 nullptr。
