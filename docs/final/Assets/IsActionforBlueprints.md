...
返回这个动作是否被设计为专门针对蓝图（true）或针对所有资产（false）。如果为真，GetSupportedClass()将被视为针对选定蓝图资产的父类的过滤器。
...
返回值。布尔值。返回该动作是否被设计为专门针对蓝图（true）或针对所有资产（false）。如果为真，GetSupportedClass()将被视为针对选定蓝图资产的父类的过滤器。
