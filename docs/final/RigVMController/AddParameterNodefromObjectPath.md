...
在给定结构对象路径名称的情况下，将参数节点添加到已编辑的图形中。参数代表图形/函数的输入或输出参数。输入参数是常量值/字数。这将导致一个 NodeAdded 修改事件。

目标是 Rig VMController

## 图示

![](/uploads/projects/ue-bluprint/20221218-20420647.png)
...
返回值。Rig VMParameter Node Object Reference。在编辑的图形中添加一个参数节点，给定一个结构对象的路径名称。参数代表图形/函数的输入或输出参数。输入参数是常量值/字数。这将导致一个 NodeAdded 修改事件。
