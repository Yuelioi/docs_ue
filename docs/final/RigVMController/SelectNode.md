...
在图中选择一个单一的节点。这将导致一个NodeSelected / NodeDeselected修改事件。

目标是Rig VMController

## 图示

![]($-20221218-20432486.png)
...
返回值。布尔值。选择图形中的一个节点。这将导致一个NodeSelected / NodeDeselected修改事件。
...
Return Value: Boolean. Selects a single node in the graph.. This causes a NodeSelected / NodeDeselected modified event..
