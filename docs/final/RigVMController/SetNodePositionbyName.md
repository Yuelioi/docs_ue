...
通过名称来设置一个节点在图形中的位置。这将引起一个 NodePositionChanged 修改事件。

目标是 Rig VMController

## 图示

![](/uploads/projects/ue-bluprint/20221218-20434839.png)
...
返回值。布尔值。通过名称设置节点在图形中的位置。这将引起一个 NodePositionChanged 修改事件。
...
Return Value: Boolean. Sets the position of a node in the graph by name.. This causes a NodePositionChanged modified event..
