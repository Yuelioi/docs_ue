...
在编辑过的图形中添加一个注释节点。注释可以用来注释图形。这将导致一个 NodeAdded 修改事件。

目标是 Rig VMController

## 图示

![](/uploads/projects/ue-bluprint/20221218-20414971.png)
...
返回值。Rig VMComment Node Object Reference。在编辑过的图形中添加一个注释节点。注释可以用来对图形进行注释。这将导致一个 NodeAdded 修改事件。
...
Return Value: Rig VMComment Node Object Reference. Adds a Comment Node to the edited Graph.. Comments can be used to annotate the Graph.. This causes a NodeAdded modified event..
