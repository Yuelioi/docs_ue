...
在编辑的图中添加一个变量节点。变量代表函数的局部工作状态，可以被读出和写入。这将导致一个 NodeAdded 修改事件。

目标是 Rig VMController

## 图示

![](/uploads/projects/ue-bluprint/20221218-20421733.png)
...
返回值。Rig VMVariable Node Object Reference。在编辑的图形中添加一个变量节点。变量代表函数的本地工作状态，可以被读出和写入。这将导致一个 NodeAdded 修改事件。
