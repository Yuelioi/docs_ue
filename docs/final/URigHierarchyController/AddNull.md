...
![](/uploads/projects/ue-bluprint/20221218-21203159.png)
...
在父母。Rig Element Key Structure.新 null 的（可选）父级。如果你不需要一个父级，请传递 FRigElementKey()。

在转型。变换。新的空值的变换--在本地或全局空值中，基于 bTransformInGlobal。

Transform in Global：布尔值。如果传递的 Transform 以全局 null 表示，则将此设置为 true，如果以局部 null 表示，则设置为 false。
...
返回值。Rig Element Key 结构。新创建的空元素的密钥。
