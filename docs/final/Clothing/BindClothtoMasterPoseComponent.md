...
如果这个组件有一个有效的 MasterPoseComponent，那么这个函数就会使从属组件上的布景项目采取主组件上布景项目的变换，而不是单独模拟。注意：这将迫使主组件上的任何布景角色在本地空间进行模拟。另外。组件中使用的网格必须是相同的，这样布才能正确地绑定。

目标是骨架网格组件

## 图示

![](/uploads/projects/ue-bluprint/20221218-18174118.png)
