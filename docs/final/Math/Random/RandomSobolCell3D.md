...
![](/uploads/projects/ue-bluprint/20221218-19535816.png)

## Inputs

索引。整数。单元中的哪个顺序点（从 0 开始）。

Num Cells:整数。单元网格的大小，1 到 1024。四舍五入到下一个 2 的幂。

单元：向量。从这个整数的网格单元给出一个点。

种子：向量。随机三维种子（范围为 0-1 的成分），在多个序列中随机化。

## Outputs

返回值。矢量。在给定网格单元中的 Sobol 分布的随机三维矢量。
