# CrossProduct

返回两个三维向量的交叉积--见http://mathworld.wolfram.com/CrossProduct.html

目标是 Kismet 数学图书馆

## 图示

![](/uploads/projects/ue-bluprint/20221218-19562826.png)

## Inputs

答：向量。

B: 向量。

## Outputs

返回值。矢量。返回两个三维向量的交叉积--见http://mathworld.wolfram.com/CrossProduct.html。

<hr>

Returns the cross product of two 3d vectors - see http://mathworld.wolfram.com/CrossProduct.html

Target is Kismet Math Library

## 图示

![](/uploads/projects/ue-bluprint/20221218-19562826.png)

## Inputs

A: Vector.

B: Vector.

## Outputs

Return Value: Vector. Returns the cross product of two 3d vectors - see http://mathworld.wolfram.com/CrossProduct.html.
