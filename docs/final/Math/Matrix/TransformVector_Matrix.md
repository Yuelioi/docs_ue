...
变换一个方向矢量--不会考虑到 FMatrix 的平移部分。如果你想变换一个表面的法线（或平面），并正确考虑非均匀缩放，你应该使用 TransformByUsingAdjointT...（假设矩阵代表一个变换）

目标是 Kismet 数学图书馆

## 图示

![](/uploads/projects/ue-bluprint/20221218-19523980.png)
...
返回值。矢量 4 结构。变换一个方向向量 - 不会考虑到 FMatrix.的平移部分。如果你想变换一个表面法线（或平面）并正确考虑非均匀缩放，你应该使用 TransformByUsingAdjointT.（假设矩阵代表一个变换）。
