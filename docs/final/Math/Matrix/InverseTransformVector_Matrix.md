...
用这个矩阵的逆值变换一个方向向量--不会考虑平移部分。如果你想对一个表面的法线（或平面）进行变换，并正确考虑非均匀缩放，你应该使用 TransformByUsingAdjointT 与矩阵逆的邻接点。

目标是 Kismet 数学图书馆

## 图示

![](/uploads/projects/ue-bluprint/20221218-19522575.png)
...
返回值。矢量。用这个矩阵的逆值变换一个方向向量--不会考虑平移部分。如果你想变换一个表面的法线（或平面），并正确考虑非均匀缩放，你应该使用 TransformByUsingAdjointT 与矩阵逆的邻接点（假设矩阵代表一个变换）。
