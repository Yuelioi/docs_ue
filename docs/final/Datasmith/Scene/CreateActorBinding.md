# CreateActorBinding

创建一个新的角色绑定，并将其添加到该变体中

目标是 Datasmith 变体元素

## 图示

![](/uploads/projects/ue-bluprint/20221218-18393967.png)

## Inputs

在。执行。

目标。Datasmith Variant Element Object Reference。

## Outputs

出：执行。

返回值。Datasmith Actor Binding Element Object Reference。创建一个新的演员绑定，并将其添加到这个变体中。

<hr>

Create a new actor binding and add it to this variant

Target is Datasmith Variant Element

## 图示

![](/uploads/projects/ue-bluprint/20221218-18393967.png)

## Inputs

In: Exec.

Target: Datasmith Variant Element Object Reference.

## Outputs

Out: Exec.

Return Value: Datasmith Actor Binding Element Object Reference. Create a new actor binding and add it to this variant.
