...
获取此属性的记录值为一个 int 点。如果该属性不是 int 点类型的，返回的值就没有意义。

目标是 Datasmith 属性捕获元素

## 图示

![](/uploads/projects/ue-bluprint/20221218-18380394.png)

## Inputs

在。执行。

目标。Datasmith 属性捕捉元素对象参考。

## Outputs

输出。输出：执行。

返回值。Int 点结构。获取该属性的记录值，作为一个 int 点。如果该属性不是 int 点类型，则返回值没有意义。
